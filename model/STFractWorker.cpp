...
{
    const calc_options &options = m_context->get_options();
    if (iter == -1) // this is a convention => numIters are set to -1 by pointFunc in case fate == FATE_INSIDE
    {
        // currently inside
...
void STFractWorker::pixel(int x, int y, int w, int h)
{
    ...
    {
        int iter = 0;
        switch (options.render_type) { // we could use the strategy pattern here having different implementations
        case RENDER_TWO_D:
        {
    ...
    {
        // do we really need this instead using the m_im->get directly ?
...
void STFractWorker::box_row(int w, int y, int rsize)
{
    ...
    // is this really necessary since those are going to be calculated by box_row ??
    ...
    else
    {
        ...
        else
        {
            if (rsize > 4)
            {
                ...
                // todo: some of the edges os this sub-boxes are already calculated...
                // that calculations are skipped by pixel() function
